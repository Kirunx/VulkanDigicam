...
class VpTexture {
 public:
  VpTexture(VpDevice &device, const std::string &textureFilepath);
  VpTexture(
      VpDevice &device,
      VkFormat format,
      VkExtent3D extent,
  ...
  VpTexture(const VpTexture &) = delete;
  VpTexture &operator=(const VpTexture &) = delete;
  ...
  static std::unique_ptr<VpTexture> createTextureFromFile(
