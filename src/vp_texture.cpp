...
namespace vp {
VpTexture::VpTexture(VpDevice &device, const std::string &textureFilepath) : mDevice{device} {
...
std::unique_ptr<VpTexture> VpTexture::createTextureFromFile(
    VpDevice &device, const std::string &filepath) {
  return std::make_unique<VpTexture>(device, filepath);
}

void VpTexture::updateDescriptor() {
  mDescriptor.sampler = mTextureSampler;
  mDescriptor.imageView = mTextureImageView;
  mDescriptor.imageLayout = mTextureLayout;
}

void VpTexture::createTextureImage(const std::string &filepath) {
...
void VpTexture::createTextureImageView(VkImageViewType viewType) {
  VkImageViewCreateInfo viewInfo{};
  viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
  viewInfo.image = mTextureImage;
...
void VpTexture::createTextureSampler() {
  VkSamplerCreateInfo samplerInfo{};
  samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
  samplerInfo.magFilter = VK_FILTER_LINEAR;
  samplerInfo.minFilter = VK_FILTER_LINEAR;
...
void VpTexture::transitionLayout(
